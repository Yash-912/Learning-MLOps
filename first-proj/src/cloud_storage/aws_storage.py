...
class SimpleStorageService:
    ...
    def __init__(self):
        try:
            s3_client = S3Client()
            self.s3_client = s3_client.s3_client
            logging.info("Initialized SimpleStorageService with S3 client")
        except Exception as e:
            raise MyException(e, sys)
    ...
    def upload_file(
        ...
        """
        Upload file to S3 using single PUT (no multipart).
        """
        logging.info(
            f"Uploading {from_filename} to s3://{bucket_name}/{to_filename}"
        )

        try:
            with open(from_filename, "rb") as file_obj:
            ...
            raise MyException(
                f"Failed to upload {from_filename} to {bucket_name}/{to_filename}: {e}",
                sys
            )
    ...
    # ------------------------------------------------------------------
    # FOLDER (OPTIONAL, SAFE)
    # ------------------------------------------------------------------

    def create_folder(self, folder_name: str, bucket_name: str):
        """
        Create an S3 folder (key ending with '/').
        """
        try:
            ...
            logging.info(f"Created folder s3://{bucket_name}/{key}")
        except ClientError as e:
            raise MyException(e, sys)
